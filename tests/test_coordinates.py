...
@pytest.mark.asyncio
async def test_returns_found_coordinates_async(mock_api: typing.Any) -> None:
    mock_api("coords_found", 200, geocode="Москва Льва Толстого 16")
    client = Client("well-known-key")

    assert await client.aiocoordinates("Москва Льва Толстого 16") == (
        Decimal("37.587093"),
        Decimal("55.733969"),
    )
...
@pytest.mark.asyncio
async def test_raises_if_coordinates_not_found_async(mock_api: typing.Any) -> None:
    mock_api("coords_not_found", 200, geocode="абырвалг")
    client = Client("well-known-key")

    with pytest.raises(NothingFound, match='Nothing found for "абырвалг"'):
        await client.aiocoordinates("абырвалг")
...
async def test_raises_for_invalid_api_key_async(mock_api: typing.Any) -> None:
    ...
    with pytest.raises(InvalidKey):
        await client.aiocoordinates("Москва Льва Толстого 16")
