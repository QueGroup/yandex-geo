...
@pytest.mark.asyncio
async def test_returns_found_address_async(mock_api: typing.Any) -> None:
    mock_api("address_found", 200, geocode="37.587093,55.733969")
    client = Client("well-known-key")

    resp = await client.aioaddress(Decimal("37.587093"), Decimal("55.733969"))
...
@pytest.mark.asyncio
async def test_raises_if_address_not_found_async(mock_api: typing.Any) -> None:
    mock_api("address_not_found", 200, geocode="337.587093,55.733969")
    client = Client("well-known-key")

    with pytest.raises(NothingFound, match='Nothing found for "337.587093 55.733969"'):
        await client.aioaddress(Decimal("337.587093"), Decimal("55.733969"))
...
@pytest.mark.asyncio
async def test_raises_for_invalid_api_key_async(mock_api: typing.Any) -> None:
    mock_api(
        {"statusCode": 403, "error": "Forbidden", "message": "Invalid key"},
        403,
        geocode="37.587093,55.733969",
    ...
    with pytest.raises(InvalidKey):
        await client.aioaddress(Decimal("37.587093"), Decimal("55.733969"))
...
def test_raises_for_unknown_response(mock_api: typing.Any) -> None:
    ...
    assert "status_code=500, body=b'{}'" in exc_info.value.args[0]


@pytest.mark.asyncio
async def test_raises_for_unknown_response_async(mock_api: typing.Any) -> None:
    mock_api({}, 500, geocode="37.587093,55.733969")
    client = Client("well-known-key")
